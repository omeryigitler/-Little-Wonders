  ...
  const cardContent = (
    ...
    >
      ...
      <div className="relative z-10 flex w-full max-w-[250px] flex-col justify-center self-start px-2 md:max-w-none md:self-center md:pl-2 md:pr-3">
        <h3 className="mb-2 font-serif text-[1.35rem] font-medium leading-tight text-[#3a251a] md:text-[1.08rem] xl:text-[1.18rem]">{product.name}</h3>
        <p className="mb-3 text-sm font-medium leading-relaxed text-[#5a4234] opacity-90 md:min-h-[42px] md:text-[12px] xl:text-[13px]">{product.description}</p>
        <p className="mb-4 font-sans text-lg font-bold text-[#3a251a] md:mb-3 md:text-[1.05rem] xl:text-lg">${product.price.toFixed(2)}</p>
        <button type="button" onClick={handleAction} disabled={preview} className={`group/btn relative flex h-11 w-[168px] items-center justify-center self-center transition-transform md:h-10 md:w-[148px] md:self-start xl:h-11 xl:w-[168px] ${preview ? 'cursor-default' : 'cursor-pointer hover:-translate-y-0.5'}`}>
