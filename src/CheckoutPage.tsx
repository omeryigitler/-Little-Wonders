...
import { ArrowLeft, CheckCircle2, Copy, Lock, ShoppingBag, X } from 'lucide-react';
      ...
      {orderReference && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#3a251a]/25 px-4 backdrop-blur-sm">
          <div className="relative w-full max-w-md rounded-[2rem] border border-boutique-brown/10 bg-[#fcfaf6] p-7 text-center shadow-2xl">
            <button onClick={closeSuccessModal} className="absolute right-4 top-4 rounded-full p-2 text-boutique-brown-light hover:bg-boutique-wood/10">
              <X className="h-5 w-5" />
            </button>
            ...
            <h2 className="font-serif text-3xl text-boutique-brown">Order Started</h2>
            <p className="mt-2 text-sm leading-relaxed text-boutique-brown-light">
              Your order reference is ready. Copy it and keep it for your records.
            </p>

            <div className="mt-6 rounded-2xl border border-boutique-brown/10 bg-white p-4">
              <p className="text-[11px] font-bold uppercase tracking-wider text-boutique-brown/60">Order Reference</p>
              <div className="mt-2 flex items-center gap-2 rounded-xl bg-boutique-bg px-3 py-3">
                <code className="flex-1 select-all text-left text-sm font-bold tracking-wide text-boutique-brown">{orderReference}</code>
                <button onClick={copyOrderReference} className="inline-flex items-center gap-1 rounded-full bg-boutique-brown px-3 py-2 text-xs font-bold text-white hover:bg-boutique-wood">
            ...
            <div className="mt-6 flex flex-col gap-3">
              <button onClick={closeSuccessModal} className="rounded-full bg-boutique-brown px-5 py-3 text-sm font-bold text-white hover:bg-boutique-wood">
                Continue Shopping
              </button>
              <p className="text-xs text-boutique-brown-light">Stripe payment connection is the next setup step.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <main className="relative z-10 mx-auto grid max-w-6xl gap-8 px-6 py-10 md:grid-cols-[58%_42%] md:py-14">
        <form onSubmit={handleSubmit} className="space-y-6 rounded-[2rem] border border-boutique-brown/10 bg-white/75 p-6 shadow-sm md:p-8">
          <div>
            ...
            <p className="mt-2 text-sm text-boutique-brown-light">Enter your shipping details to prepare your personalized gift order.</p>
          </div>
          ...
          <button disabled={isSubmitting || cartItems.length === 0} className="w-full rounded-full bg-boutique-brown px-6 py-4 text-base font-bold text-white shadow-sm transition-transform hover:-translate-y-0.5 hover:bg-boutique-wood disabled:cursor-not-allowed disabled:opacity-50">
        ...
        <aside className="rounded-[2rem] border border-boutique-brown/10 bg-white/75 p-6 shadow-sm md:p-8 h-fit">
          <div className="mb-6 flex items-center gap-2">
            ...
            <h2 className="font-serif text-2xl text-boutique-brown">Order Summary</h2>
          </div>

          {cartItems.length === 0 ? (
            <div className="rounded-2xl bg-boutique-bg p-6 text-center text-sm text-boutique-brown-light">Your gift bag is empty.</div>
          ) : (
            <div className="space-y-4">
              {cartItems.map((item) => (
                <div key={item.id} className="flex gap-4 rounded-2xl bg-boutique-bg p-3">
                  <div className="relative h-20 w-20 flex-shrink-0 overflow-hidden rounded-xl bg-white">
                    {item.product.bgImage && <img src={item.product.bgImage} className="absolute inset-0 h-full w-full object-cover opacity-70" alt="" />}
                    <img src={item.product.imageUrl} className="relative z-10 h-full w-full object-contain p-2" alt="" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-serif text-sm font-medium text-boutique-brown">{item.product.name}</p>
                    <p className="mt-1 text-xs text-boutique-brown-light">Qty {item.quantity}</p>
                    {Object.keys(item.personalizationData).length > 0 && <p className="mt-1 truncate text-[11px] text-boutique-brown-light">Personalized</p>}
                  </div>
                  <p className="text-sm font-bold text-boutique-brown">${(item.product.price * item.quantity).toFixed(2)}</p>
                </div>
              ))}
            </div>
          )}

          <div className="mt-6 space-y-3 border-t border-boutique-brown/10 pt-5 text-sm">
            <div className="flex justify-between text-boutique-brown-light"><span>Subtotal</span><span>${subtotal.toFixed(2)}</span></div>
            <div className="flex justify-between text-boutique-brown-light"><span>Shipping</span><span>${shipping.toFixed(2)}</span></div>
            <div className="flex justify-between pt-3 font-serif text-xl text-boutique-brown"><span>Total</span><span>${total.toFixed(2)}</span></div>
          </div>
        </aside>
      </main>
    </div>
